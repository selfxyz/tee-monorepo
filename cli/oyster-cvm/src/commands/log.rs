...
async fn ping_check(ip: &str) -> Result<()> {
    match timeout(PING_TIMEOUT, Command::new("ping")
        .arg("-c")
        .arg("1")
        .arg(ip)
        ...
        .status()).await {
            ...
            _ => anyhow::bail!("Failed to ping {} (timeout after {} seconds)", ip, PING_TIMEOUT.as_secs())
    }
}

pub async fn stream_logs(ip: &str, start_from: Option<&str>, with_log_id: bool, quiet: bool) -> Result<()> {
    ping_check(ip).await?;
    ...
    let response = match timeout(STREAM_CONNECT_TIMEOUT, client.get(&url).send()).await {
        ...
        Err(_) => anyhow::bail!("Connection to console stream timed out after {} seconds", 
