...
pub fn get_attestation_doc(public_key: &[u8], user_data: &[u8]) -> Result<Vec<u8>, String> {
    if public_key.len() > u16::MAX as usize {
        return Err("public key is too long, maximum of 65535".into());
    }

    if user_data.len() > u16::MAX as usize {
    ...
    offset += 10;
    offset += encode(&mut attestation[offset..], Some(user_data));
    attestation[offset..offset + 6].copy_from_slice(b"\x65nonce");
    offset += 6;
    offset += encode(&mut attestation[offset..], None);
...
pub fn get_hex_attestation_doc(public_key: &[u8], user_data: &[u8]) -> Result<String, String> {
    let attestation = get_attestation_doc(public_key, user_data);
