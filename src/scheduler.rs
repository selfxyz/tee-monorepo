...
use crate::utils::{
    check_and_delete_file, AppState, SecretMetadata, DOMAIN_SEPARATOR, GARBAGE_CLEAN_JOB_INTERVAL,
    SECRET_EXPIRATION_BUFFER, SEND_TRANSACTION_BUFFER,
};
...
pub async fn garbage_cleaner(app_state: Data<AppState>) {
    ...
    let mut interval = interval(Duration::from_secs(GARBAGE_CLEAN_JOB_INTERVAL));
        ...
        for (secret_id, secret_metadata) in secrets_stored {
            ...
            if SystemTime::now()
                > SystemTime::UNIX_EPOCH
                    + Duration::from_secs(
                        secret_metadata.end_timestamp.to::<u64>() + SECRET_EXPIRATION_BUFFER,
                    )
            {
